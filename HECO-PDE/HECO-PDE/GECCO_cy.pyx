...
#        _Q = sorted(_Q, key=operator.attrgetter('obj'))
    ...
    cdef void crossover_2(self, list QParent, list C_X, int D, double cr, int idx):
        ...
        while random.uniform(0,1) <= cr and L < D - 1:
            ...
            L += 1  
    cdef pca_search(self, list Q, list C_X, int Lambda, int D, int idx):
        cdef:
            list V, V_X, Temp
        
        V = []
        [V.append(item) for item in Q]
        V_X = []
        V_X.append(C_X)
        ...
        tf_V_X = pca.fit_transform(V_X)
        pj_V_X = pca.inverse_transform(tf_V_X)
        C_X = pj_V_X[idx].tolist()
        return C_X
            
    cdef list DE(self, list P, list A, int NP, int D, double cr, double f, int Stg, int index, double grate, int jugg, double p_pca):
        cdef:
            list C_X
        C_X = []
        if Stg == 0:
            C_X = self.mutation_1(P, A, NP, D, f, index, grate, jugg)
            if random.random() < p_pca:
                self.crossover_1(P, C_X, D, cr, index)
            else:
                self.pca_search(P, C_X, NP, D, index)
        elif Stg == 1:
            C_X = self.mutation_1(P, A, NP, D, f, index, grate, jugg)
            if random.random() < p_pca:
...
#        return para**(20 * (idx+ 1)) * eq + (1 - w_t) * (h1 ) ** w_i  +  w_t * (h2) ** w_i
        return w_t**(30.0 * w_i) * eq + (1 - w_i) * (1 - w_t)** (w_i * 5.0)  * (h1 )  +  w_i * w_t** (w_i * 5.0)  * (h2)
    ...
    cdef _optimize(self, int benchID, int D):
        ...
        while FES < FES_MAX * D:
            ...
            for idx in range(Lambda):
                ...
                cr = f_cr[0]
                f = f_cr[1]
                QChild_X.append(self.DE(QParent, A, Lambda, D, cr, f, Stg, idx, para, jugg, p_pca))
